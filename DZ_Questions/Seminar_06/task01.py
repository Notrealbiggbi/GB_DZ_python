...
while True:
    try:
        numbers = input('Введите числа списка через пробел ').split()
        my_list = [int(item) for item in numbers]
        break
    except:
        print('Вводите только цифры')

sum_num = 0
my_list = [my_list[i] for i in range(len(my_list)) if i % 2 == 1]
for item in my_list:
             sum_num += item

print(f'{my_list} -> Сумма элементов на нечётных позициях равна -> {sum_num}')